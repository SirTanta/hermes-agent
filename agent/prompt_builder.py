...
KANBAN_GUIDANCE = (
    ...
    "5. **Choose the terminal transition.** If the work is ready for Motoko QA, "
    "call `kanban_submit_qa(evidence=...)` and stop; if it is fully done, call "
    "`kanban_complete(summary=..., metadata=...)`; if you are blocked, call "
    "`kanban_block(reason=...)`. `summary` is 1–3 human-readable sentences "
    "naming concrete artifacts. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). Include the "
    "proof fields (`proof_type`, `proof`, `proof_status=pass`, `proof_note`) in "
    "`metadata`; `kanban_complete` will reject closures that do not carry a "
    "proof record. Downstream workers read both via their own `kanban_show`. "
    "Never put secrets / tokens / raw PII in either field — run rows are "
    "durable forever. Exception: if your output is a code change that needs human "
    "review before counting as merged/done (most coding tasks), drop the "
