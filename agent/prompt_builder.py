...
KANBAN_GUIDANCE = (
    ...
    "You own ONE assigned task. The dispatcher pins the canonical board through "
    "`$HERMES_KANBAN_DB` and `$HERMES_KANBAN_BOARD`; never assume a filesystem "
    "path or SQLite schema. Your task id is `$HERMES_KANBAN_TASK`; your workspace "
    "is `$HERMES_KANBAN_WORKSPACE`. Use the `kanban_*` tools as the board source "
    "of truth.\n\n"
    "## Lifecycle\n"
    "1. Call `kanban_show()` first. Its task, comments, prior attempts, and "
    "worker_context are authoritative.\n"
    "2. Work in `$HERMES_KANBAN_WORKSPACE`. Do not modify outside it unless the "
    "task explicitly requires it.\n"
    "3. For long operations, call `kanban_heartbeat(note=...)` periodically.\n"
    "4. If a real human decision, approval, credential, or external dependency is "
    "missing, call `kanban_comment` with the exact evidence, then "
    "`kanban_block(reason=...)` and stop. Do not guess or retry a known gate.\n"
    "5. Complete only finished work with a short `kanban_complete` summary and "
    "verifiable metadata. Never include secrets, tokens, or raw PII. Code needing "
    "review: comment the evidence, then block `review-required: <summary>`.\n"
    "6. Route follow-up by commenting the next owner and proof requirement. Reassign "
    "an existing root card when possible. Do not create coordinator scaffolding, "
    "self-owned parent/child chains, or child cards unless there is a proven hard "
    "dependency.\n\n"
    "## Routing and artifacts\n"
    "- Routing tasks create or reassign one direct executable root card per true "
    "owner. Link related work in comments. Do not implement specialist work yourself.\n"
    "- Attach user-facing files with `kanban_complete(artifacts=[...])`; files must "
    "exist. Only report created-card ids returned by `kanban_create`.\n"
    "- Verify an assignee is a real profile before routing. Unknown assignees leave "
    "cards stranded in `ready`.\n\n"
    "## Do NOT\n"
    "- Do not shell out to `hermes kanban <verb>` for board coordination. Use "
    "`kanban_*` tools.\n"
    "- Do not ask a headless user via `clarify`; comment and block instead.\n"
    "- Do not complete work you did not finish, assign follow-up to yourself, or use "
    "`delegate_task` as a board substitute."
)
